    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        ...
        <li aria-current="page">
            <div class="flex items-center">
                ...
                <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2">Đơn hàng</span>
            </div>
        </li>
    </ol>
</nav>
        ...
        <div class="relative overflow-hidden bg-white shadow-md sm:rounded-lg">
            <div class="flex flex-col px-4 py-3 space-y-3 lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:space-x-4">
                <div class="flex items-center flex-1 space-x-4">
                    <h2 class="text-black text-2xl font-semibold">Danh sách đơn hàng</h2>
                </div>
                <div class="flex flex-col flex-shrink-0 space-y-3 md:flex-row md:items-center lg:justify-end md:space-y-0 md:space-x-3">
                    <a href="{{ route('order.create') }}" type="button" class="flex items-center justify-center px-4 py-2 text-sm font-medium text-white rounded-lg bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3">Người đặt hàng</th>
                            <th scope="col" class="px-4 py-3">Ngày đặt hàng</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $order->id }}</th>
                            <td class="px-4 py-3">{{ $order->order_code }}</td>
                            <td class="px-4 py-3">{{ $order->orderUser->name }}</td>
                            ...
                            <td class="px-4 py-3 flex items-center justify-end">
                                @include('admin.order.partials.show-order')
                                <a href="{{ route('order.edit', $order->id) }}" class="block p-2">
                                ...
                                {{-- <form class="py-1" action="{{ route('order.destroy', $order->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-full text-left block py-2 px-2 text-base" onclick="return confirm('Bạn chắc chắn muốn xóa ?')">
