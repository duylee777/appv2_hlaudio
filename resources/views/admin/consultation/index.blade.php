...
@section('title', 'Khách hàng đăng ký nhận tin')
    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        ...
        <li aria-current="page">
            <div class="flex items-center">
                ...
                <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2">Danh sách đăng ký nhận tin</span>
            </div>
        </li>
    </ol>
</nav>
        ...
        <div class="relative overflow-hidden bg-white shadow-md sm:rounded-lg">
            <div class="flex flex-col px-4 py-3 space-y-3 lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:space-x-4">
                <div class="flex items-center flex-1 space-x-4">
                    <h2 class="text-black text-2xl font-semibold">Danh sách đăng ký nhận tin</h2>
                </div>

                <!-- Search form -->
                <button id="reset-filter" data-url={{route('admin.consultation')}} class="flex items-center gap-2 group">
            ...
            <div class="overflow-x-auto">
                <div class="flex flex-wrap items-center gap-4 justify-end px-4 mb-4">
                    <div class="grow">
                        <label for="" class="text-xs font-semibold text-blue-400">Tìm kiếm email</label>
                    ...
                    <div class="grow">
                        ...
                        <div class="relative max-w-sm">
                            ...
                            {{-- <input datepicker id="filter-created" name="filter-created"  type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5" placeholder="Chọn ngày" value="{{ $created == 'all' ? '' : $created }}"> --}}
                    ...
                    <tbody>
                        @if(isset($consultations) && count($consultations) != 0)
                            ...
                            <tr class="bg-white border-b">
                                ...
                                <td class="px-4 py-3">
                                    @if($consultation->status == config('global.consultation_status.new'))
                                        <span class="block w-max text-xs text-white bg-red-500 px-2 py-1 rounded">{{ $consultation->status }}</span>
                                    @endif
                                    @if($consultation->status == config('global.consultation_status.unread'))
                                        <span class="block w-max text-xs text-white bg-yellow-500 px-2 py-1 rounded">{{ $consultation->status }}</span>
                                    @endif
                                    @if($consultation->status == config('global.consultation_status.read'))
                                        <span class="block w-max text-xs text-white bg-blue-500 px-2 py-1 rounded">{{ $consultation->status }}</span>
                                    @endif
                                    
                                </td>
                                <td class="px-4 py-3">
                                    @if($consultation->status != config('global.consultation_status.read'))
                                        <form data-id="{{ $consultation->id }}" data-action="{{ route('admin.consultation.update') }}" class="form-read-consultation">
                                            <input id="checked-checkbox-{{$consultation->id}}" type="checkbox" value="read" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 hover:bg-blue-500 hover:border-blue-500 cursor-pointer">
                                        </form>
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-4">
                                        <button class="delete-consultation" data-route="{{route('admin.consultation.destroy',$consultation->id)}}">
...
<script>
    $(document).ready(function () {
        ...
        let formReadConsultations = $('.form-read-consultation');
        $.each(formReadConsultations, function() {
        ...
        $.each($('.delete-consultation'), function() {
            $(this).on("click", function(e) {
                e.preventDefault();
                Swal.fire({
                title: "Bạn có chắc chắn muốn email này?",
                icon: "warning",
